...
链接：https://leetcode-cn.com/problems/max-increase-to-keep-city-skyline
    ...
    Solution temp;
    int ret = temp.maxIncreaseKeepingSkyline(grid);
    cout << ret << endl;
...
public:
    ...
    int maxIncreaseKeepingSkyline(vector<vector<int>>& grid) {
        ...
        int ret = 0;
        vector<int> max_row(len_m);
        for (int i = 0; i < len_m; ++i)
            max_row[i] = *max_element(grid[i].begin(), grid[i].end());
        for (int j = 0; j < len_n; ++j) {
            int max_temp = grid[0][j];
            for (int i = 1; i < len_m; ++i)
                max_temp = max(max_temp, grid[i][j]);
            for (int i = 0; i < len_m; ++i)
                ret += min(max_row[i], max_temp) - grid[i][j];
        }
        return ret;
    }
};